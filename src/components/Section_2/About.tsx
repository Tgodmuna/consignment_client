...
const About = () => {
  ...
  const reasonsCard = reasons.map((item, index): JSX.Element => {
    return (
      <li
        key={index}
        className=' shadow-red-300 shadow-sm max-w-full max-h-fit rounded-lg border border-gray-400 border-l-red-500 border-l-8 bg-slate-100 my-2 p-2 flex gap-x-1 '>
        <img src={item.icon} alt='icon' className='' />
        <div className='flex flex-col gap-y-1'>
          <p className='self-end text-neutral-900 text-lg font-bold'>
            {item.title}
          </p>
          <p className='text-xs w-fit h-fit text-gray-400 capitalize'>
            {item.text}
          </p>
        </div>
      </li>
    ...
    <section
      ...
      <div className='container m-2 pb-4'>
        ...
        <p
          ...
          solution for all your logistics needs. Our extensive range of services
          encompasses logistics, warehousing, air freight, sea freight, rail
          freight, ocean freight, and cargo insurance. With a commitment to
          excellence and reliability, we have become a trusted partner for
          businesses worldwide. Our journey began in the UK, and since then, we
          have grown exponentially, expanding our operations to the United
          States and establishing numerous branches across Europe and Asia.
          <img
        ...
        <p
          ...
          Whether it's transporting goods via air, sea, rail, or ocean, we have
